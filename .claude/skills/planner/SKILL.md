...
2b. Read boyscout output — the boyscout skill already ran before the planner.
   Read `memory/boyscout_review.json` and `docs/diagrams/` to understand the
   current state of each deliverable file.

   If `scope_additions` is non-empty in `boyscout_review.json`, include each
   addition as an extra task in the plan (human already approved these).
